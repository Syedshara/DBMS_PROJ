...
app.get('/api/job_cv', async (req, res) => {

  try {
    const result = await pool.query('SELECT client_id,client_name FROM client order by client_id');
    const result1 = await pool.query('SELECT vendor_id,vendor_name FROM vendor order by vendor_id');
    res.json({
      client: result.rows,
      vendor: result1.rows,
    });
...
app.get('/api/candidate_cv', async (req, res) => {
...
app.get('/api/home/candidate_status', async (req, res) => {
  try {

    const result1 = await pool.query('SELECT * FROM candidate_status');
    const result2 = await pool.query('SELECT j.job_code, j.posting_title, j.vendor_id, v.vendor_name FROM job_opening j INNER JOIN vendor v USING (vendor_id)');
    const result3 = await pool.query('SELECT * FROM client');
    res.json({
      data1: result1.rows,
      data2: result2.rows,
      data3: result3.rows,
...
app.get('/api/home', async (req, res) => {
  try {
    const jobOpeningResult = await pool.query('SELECT job_code, posting_title FROM job_opening');
    const candidateResult = await pool.query('SELECT resume_id, first_name FROM candidate');
    const vendor = await pool.query('SELECT vendor_id,vendor_name FROM vendor'); 

    res.json({
      jobOpening: jobOpeningResult.rows,
...
app.put('/api/home/candidate_status', async (req, res) => {

  const {job,id,name,status,title} = req.body;  // Directly extract the new password from the request body
  try {
    const result = await pool.query('insert into candidate_status values($1,$2,$3,$4,$5)', [job, id,status,name,title]);
    res.json(result.rows);
  } catch (error) {
    console.error('Error executing query', error);
    res.status(500).json({ error: 'Internal server error' });
...
app.put('/api/home/client', async (req, res) => {

  const {clientName,clientStatus,mobile,email} = req.body;  // Directly extract the new password from the request body
  try {
    const result = await pool.query('insert into client values($1,$2,$3,$4)', [clientName, clientStatus,mobile,email]);
    res.json(result.rows);
  } catch (error) {
    console.error('Error executing query', error);
    res.status(500).json({ error: 'Internal server error' });
...
app.put('/api/home/vendor_job', async (req, res) => {

  const {job,id,name,title} = req.body;  // Directly extract the new password from the request body
  try {
    const result = await pool.query('insert into offered_by values($1,$2,$3,$4)', [job, title,id,name]);
    res.json(result.rows);
  } catch (error) {
    console.error('Error executing query', error);
    res.status(500).json({ error: 'Internal server error' });
...
app.put('/api/login/:user', async (req, res) => {
  const { user } = req.params;
  const {newPass} = req.body;  // Directly extract the new password from the request body
  try {
    const result = await pool.query('UPDATE users SET password = $1 WHERE username = $2', [newPass, user]);
    res.json(result.rows);
  } catch (error) {
    console.error('Error executing query', error);
    res.status(500).json({ error: 'Internal server error' });
...
app.put('/api/client/:idy', async (req, res) => {
  const { idy } = req.params;
  const {clientName,clientStatus,mobile,email} = req.body;  // Directly extract the new password from the request body
  try {
    const result = await pool.query('UPDATE client SET client_id = $1,client_name=$2,mobile=$3,email=$4 WHERE off_id = $5', [clientName, clientStatus,mobile,email,idy]);
    res.json(result.rows);
  } catch (error) {
    console.error('Error executing query', error);
    res.status(500).json({ error: 'Internal server error' });
...
app.put('/api/job_opening/update/:id', async (req, res) => {
  const { id } = req.params;
  ...
  try {
    const result = await pool.query(
      ...
      [
        ...
        updatedData.v10,
        updatedData.v11,
        updatedData.editedOption1,
        updatedData.editedOption,
        id
      ]
    );
